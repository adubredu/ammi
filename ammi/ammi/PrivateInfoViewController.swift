...
class PrivateInfoViewController: UIViewController {
    ...
    {
        performSegue(withIdentifier: "backToPassCodeSearch", sender: self)
    }
    
    @IBAction func noButtonIsPressed(_ sender: Any)
    {
        let ref = Database.database().reference()
        ref.child("Private_Posts").child(self.postID).observeSingleEvent(of: .value, with:
            { (snapshot) in
                ...
                if let properties = snapshot.value as? [String : AnyObject]
                {
                    if let peopleGoing = properties["peopleGoing"] as? [String : AnyObject]
                    {
                        for (id, person) in peopleGoing
                        {
                            ...
                            {
                                ref.child("Private_Posts").child(self.postID).child("peopleGoing").child(id).removeValue(
                                                ...
                                                {(snap) in
                                                    if let prop = snap.value as? [String : AnyObject]
                                                    {
                                                        if let going = prop["peopleGoing"] as? [String : AnyObject]
                                                        {
                                                            ...
                                                            ref.child("Private_Posts").child(self.postID).updateChildValues(["noGoing" : count])
                                                        }
                                                            
                                                        else
                                                        {
                                                            self.numGoing.text = "0"
                                                            ref.child("Private_Posts").child(self.postID).updateChildValues(["noGoing" : "0"])
                                                        }
                                                    }
                                            })
                                        }
    ...
    {
        let ref = Database.database().reference()
        let autoKey = ref.child("Private_Posts").childByAutoId().key
        
        ref.child("Private_Posts").child(postID).observeSingleEvent(of: .value, with: {(snapshot: DataSnapshot)
            in
            
            if snapshot.hasChildren()
            {
                let updateGoing : [String : Any] = ["peopleGoing/\(autoKey)" : self.userID!]
                ref.child("Private_Posts").child(self.postID).updateChildValues(updateGoing, withCompletionBlock: { (error, reff)
                    in
                    
                    if error == nil
                    {
                        ref.child("Private_Posts").child(self.postID).observeSingleEvent(of: .value, with: { (snap)
                            in
                            ...
                            if let properties = snap.value as? [String : AnyObject]
                            {
                                if let going = properties["peopleGoing"]  as? [String : AnyObject]
                                {
                                    ...
                                    ref.child("Private_Posts").child(self.postID).updateChildValues(updateNum)
                                }
                            }
                        })
                    }
